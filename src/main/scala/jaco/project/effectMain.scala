...
import cats.{Applicative, Bifunctor, FlatMap, Functor, Id, SemigroupK, Show, ~>}
...
import jaco.project.data.structure.SearchOperations.binary
import jaco.project.data.structure.Tree
...
object effectMain extends IOApp.Simple:
  ...
  override def run: IO[Unit] =
    ...
    given LoggerFactory[IO] = NoOpFactory.apply[IO]
    ...
    val input = (1 to 20000).toArray
    val instant = Clock[IO].realTimeInstant.flatMap(IO.println)
    for {
      _ <- instant
      _ <- instant
      _ <- IO.println(binary(input, -1, 0, 20000-1))
      _ <- instant
      _ <- IO.println(binary(input, 20000+1, 0, 20000-1))
      _ <- instant
    } yield ()
